...
class TestQuoteRouter:
    ...
    def adapter_a(self):
        ...
        adapter.fetch_quotes = AsyncMock()
        return adapter
    ...
    async def test_first_adapter_succeeds(self, adapter_a, adapter_b):
        ...
        adapter_a.fetch_quotes.return_value = [quote]

        router = QuoteRouter(adapters=[adapter_a, adapter_b])
        results = await router.fetch_quotes([code])

        assert results == [quote]
        adapter_a.fetch_quotes.assert_called_once()
        adapter_b.fetch_quotes.assert_not_called()

    @pytest.mark.asyncio
    async def test_fallback_when_first_fails(self, adapter_a, adapter_b):
        code = StockCode("600001.SH")
        quote = Quote(code=code, name="测试", price=10.0)
        adapter_a.fetch_quotes.return_value = []  # fails
        adapter_b.fetch_quotes.return_value = [quote]
        ...
        assert results == [quote]
        adapter_a.fetch_quotes.assert_called_once()
        adapter_b.fetch_quotes.assert_called_once()

    @pytest.mark.asyncio
    async def test_all_adapters_fail(self, adapter_a, adapter_b):
        code = StockCode("600001.SH")
        adapter_a.fetch_quotes.return_value = []
        adapter_b.fetch_quotes.return_value = []
